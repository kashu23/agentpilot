...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[var(--canvas)] p-2.5 text-[var(--ink)] md:p-5">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}><DialogContent className="max-w-[460px] rounded-[24px] border-black/8 bg-[#f8f9f5] p-6"><DialogHeader className="text-left"><DialogTitle className="text-xl font-semibold tracking-[-.035em]">Add a task</DialogTitle><DialogDescription className="text-xs">Human edits update the same state agents inspect.</DialogDescription></DialogHeader><form onSubmit={createTask} className="mt-5 space-y-4"><label className="block"><span className="form-label">Task title</span><Input value={newTitle} onChange={(event) => setNewTitle(event.target.value)} maxLength={100} placeholder="e.g. Production readiness review" className="mt-2 h-11 rounded-xl border-black/8 bg-white" autoFocus /></label><label className="block"><span className="form-label">Priority</span><select value={newPriority} onChange={(event) => setNewPriority(event.target.value as typeof newPriority)} className="mt-2 h-11 w-full rounded-xl border border-black/8 bg-white px-3 text-xs outline-none focus:ring-2 focus:ring-[#8bad4e]/30">{['low', 'medium', 'high', 'critical'].map((priority) => <option key={priority} value={priority}>{priority.charAt(0).toUpperCase() + priority.slice(1)}</option>)}</select></label><Button type="submit" className="h-11 w-full rounded-xl bg-[#171b18] text-white"><Plus /> Create task</Button></form></DialogContent></Dialog>
    </main>
  );
}
